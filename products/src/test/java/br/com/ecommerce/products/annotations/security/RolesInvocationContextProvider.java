package br.com.ecommerce.products.annotations.security;
...
public class RolesInvocationContextProvider implements TestTemplateInvocationContextProvider {
    ...
    public boolean supportsTestTemplate(ExtensionContext context) {
        return context.getTestMethod()
            .map(method -> method.isAnnotationPresent(ContextualizeUserWithRoles.class))
            .orElse(false);
    }

    @Override
    public Stream<TestTemplateInvocationContext> provideTestTemplateInvocationContexts(ExtensionContext context) {
        var rolesAnnotation = context.getRequiredTestMethod().getAnnotation(ContextualizeUserWithRoles.class);
